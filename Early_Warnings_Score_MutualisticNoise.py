...
with open(folderd+fn+'_var1_o.csv') as csvfile:
...
plt.subplot(4,2,1)
plt.plot(np.arange(1,0,-1/chsteps),varM100)
plt.title("$\sigma_a$ = 10")
plt.xlim(1,0)
plt.ylabel("variance")
plt.xlabel("$\mu$")
plt.subplot(4,2,2)
plt.plot(np.arange(1-win/chsteps,0,-1/chsteps),polyf100)
plt.ylabel("variance change")
plt.xlim(1-win/chsteps,0)
plt.xlabel("$\mu$")
plt.title("$\sigma_a$ = 10 ")
plt.plot([0,len(polyf100)],[0,0],'k')

for m in range(len(nums_tests)):
    k=nums_tests[m]
#get simulation data
...
#            polyf0[i,j] = np.polyfit(a,varM0[i:i+win,j],1)[0]
    ...
    plt.subplot(4,2,(2*(m+1))+1)
    plt.plot(np.arange(1,0,-1/chsteps),varM100)
    plt.title("$\sigma_m$ = "+str(sigma_m[m]))
    plt.xlim(1,0)
    plt.ylabel("variance")
    plt.xlabel("$\mu$")
    plt.subplot(4,2,(2*(m+1))+2)
    plt.plot(np.arange(1-win/chsteps,0,-1/chsteps),polyf100)
    plt.ylabel("variance change")
    plt.xlim(1-win/chsteps,0)
    plt.xlabel("$\mu$")
    ...
    plt.plot([0,len(polyf100)],[0,0],'k')
plt.tight_layout()    
#plt.savefig("mult_noise.eps",dpi=300)

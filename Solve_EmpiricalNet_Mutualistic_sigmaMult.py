...
#tch = 10 # sims run for this amount of time before changing the parameter
...
# 0: as in paper
# 1,2,3: changes in Ac (intespecific) 
# 4: delta = 1
# 5: h=0.1
# 6: h=0.001
# 7: sigmax = .1
...
nums_tests = [16,17,18] 
#
for i in range(len(nums_tests)):

    print(["test",nums_tests[i]])

#    num_test = str(i) #keep 0 for when h is the tested parameter
    num_test_p = str(0) # 0 is the set used in the paper use this with 5-9 and 0
    num_test_s = str(nums_tests[i])
#    h = htests[i]
    sigmax = sigmaxtests[i]
    print(sigmax)
    
    with open(folder+fn+'_A_'+num_test_p+'.csv') as csvfile:
                 Al = list(csv.reader(csvfile,delimiter=' ',quoting=csv.QUOTE_NONNUMERIC)) #mutualistic network
    A = np.array(Al)      

    with open(folder+fn+'_Ac_'+num_test_p+'.csv',newline='') as csvfile:
             Acl =  list(csv.reader(csvfile,delimiter=' ',quoting=csv.QUOTE_NONNUMERIC)) #competition network
    Ac = np.array(Acl)

#keep the next two with _1
...
##%% plot to check simulation results
    plt.figure()
    plt.plot(Muts1)
    plt.ylim(0,np.max(Muts1)+np.max(Muts1)*.1)
    plt.savefig(fn+"_ts1_"+num_test_s+".png", dpi=300)
    plt.close()
...
#% save data
    ...
    np.savetxt(folderd+fn+"_var1_"+num_test_s+".csv", varM)
    np.savetxt(folderd+fn+"_mean1_"+num_test_s+".csv", meanM)
    np.savetxt(folderd+fn+"_samp1_"+num_test_s+".csv", sampM)
    print(["saved",num_test_s])
#    
